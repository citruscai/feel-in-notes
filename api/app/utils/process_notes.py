...
def structure_notes(text):
    ...
    prompt_text = f"""
    Convert the following text into a structured JSON format suitable for creating fill-in-the-blank guided notes. Ensure the JSON has 'title' for section titles, 'content' for paragraph texts, and 'lists' for bullet points or numbered lists. Importantly:
    ...
    - Add new '<mark>' tags around longer, significant information in 'content' and 'items' to create meaningful fill-in-the-blank sections. Each section should include longer phrases or sentences with '<mark>' tags suitable for fill-in-the-blank notes. If a section or list item already contains markings, ensure additional significant information is also marked.
    ...
    {{
      ...
      "sections": [
        {{
          "title": "Section Title",
          "content": ["First paragraph of the section...", "<mark>Highlighted text that spans a phrase or sentence</mark> should remain unaltered."],
          "lists": [
            {{
              "list_title": "Key Points",
              "items": ["First key point with <mark>important</mark> information...", "Second key point with <mark>critical details</mark>..."]
            }}
          ]
        }},
        {{
    ...
    Given this structure, please format the following text for fill-in-the-blank guided notes, preserving all existing '<mark>' tags and ensuring that new '<mark>' tags cover longer phrases or sentences in 'content' and 'items': {text}
    """
    safety_settings = [
        {
    ...
    model = gemini.GenerativeModel(model_name='gemini-1.5-flash', safety_settings=safety_settings)
